...
with tab2:
    ...
    if option == 'Conservar el Bono':
        ...
        with col1:
            ...
            TC = st.slider('Ingresa la tasa del cupón', min_value=0.0,max_value=1.0,step=0.01)
        ...
        with col2:
            result = round((calcular_valor_presente_venta(fecha_vencimiento=fecha_vencimiento, tasa_cupon=TC, tasa_actual= TA, valor_nominal=VB)*VB)/100,2)
            st.write(calcular_valor_presente_venta(fecha_vencimiento=fecha_vencimiento, tasa_cupon=TC, tasa_actual= TA, valor_nominal=VB))
            text = "El valor presente del bono es:"
