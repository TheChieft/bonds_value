...
def B_loop(b, phi, alpha, beta, gam, lam):
    b = phi * (lam + gam) - 0.5 * gam**2 + beta * b + \
        (0.5*(phi - gam)**2)/(1 - 2 * alpha * b)
    return b
...
class HNGarch(object):
    ...
    def lr_vol(self):
        ...
        print('Annualized volatility: ' +
              str(round(sqrt(self.lr_var)*sqrt(252)*100, 4)) + '%')
        print('Daily volatility:      ' +
              str(round(sqrt(self.lr_var)*100, 4)) + '%')
    ...
    def hist_simulation(self):
        ...
        for i in range(1, len(r_t)):
            h_t = params[0] + params[2] * h_t + \
                params[1]*pow(z - params[3]*sqrt(h_t), 2)
            h_vec.append(h_t)
